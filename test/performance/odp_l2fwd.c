...
{
	uint64_t thr_maximum_pps[MAX_WORKERS] = { 0 };
	uint64_t thr_pkts_prev[MAX_WORKERS] = { 0 };
	uint64_t pkts = 0;
	...
	do {
		...
		for (i = 0; i < num_workers; i++) {
			...
			if (stats_enabled) {
				pps = (thr_stats[i].s.packets - thr_pkts_prev[i]) / timeout;
				if (pps > thr_maximum_pps[i])
					thr_maximum_pps[i] = pps;
				printf("[%02i] %9" PRIu64 " pps, %9" PRIu64 " max pps, ",
				       thr_stats[i].s.thr, pps, thr_maximum_pps[i]);

				printf(" %9" PRIu64 " rx drops, %9" PRIu64 " tx drops\n",
				       thr_stats[i].s.rx_drops, thr_stats[i].s.tx_drops);

				thr_pkts_prev[i] = thr_stats[i].s.packets;
			}
		}
		if (stats_enabled) {
			...
			if (pps > maximum_pps)
				maximum_pps = pps;
			printf("ALL: %9" PRIu64 " pps, %9" PRIu64 " max pps, ",  pps,
			       maximum_pps);

			printf(" %9" PRIu64 " rx drops, %9" PRIu64 " tx drops\n",
			       rx_drops, tx_drops);

			/* TODO: implement stats callback */
			printf("Pktio stats:\n");
			odp_pktio_stats_dump(gbl_args->pktios[0].pktio);
...
int main(int argc, char *argv[])
{
	...
	params.pkt.num     = POOL_SIZE_GLOBAL + POOL_SIZE_LOCAL * num_workers;
