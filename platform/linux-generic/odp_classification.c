...
/* ODP ThunderX pool/pool_entry one-one mapping */
#ifdef HAVE_THUNDERX
	return (odp_pool_t)cos->s.pool;
#else
...
#ifdef HAVE_THUNDERX
	if (packet_hdr_has_error(pkt_hdr))
...
#ifdef HAVE_THUNDERX
	if (packet_hdr_has_l3(hdr) && packet_hdr_has_ipv4(hdr)) {
		ipv4 = (const odph_ipv4hdr_t *)(pkt_addr + packet_l3_offset(hdr));
		dscp = ODPH_IPV4HDR_DSCP(ipv4->tos);
		cos = l3_cos->cos[dscp];
	} else if (packet_hdr_has_l3(hdr) && packet_hdr_has_ipv6(hdr)) {
		ipv6 = (const odph_ipv6hdr_t *)(pkt_addr + packet_l3_offset(hdr));
...
#ifdef HAVE_THUNDERX
	if (packet_hdr_has_l2(hdr) && packet_hdr_has_vlan(hdr) &&
	    packet_hdr_has_eth(hdr)) {
		eth = (const odph_ethhdr_t *)(pkt_addr + packet_l2_offset(hdr));
