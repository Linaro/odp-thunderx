...
{
#ifdef HAVE_THUNDERX
	pkt_hdr->hw.input_flags.l2_flags.l2_pressent = 1;
...
#ifdef HAVE_THUNDERX
	pkt_hdr->hw.error_flags.all  = 0;
	pkt_hdr->hw.input_flags.all  = 0;
	pkt_hdr->hw.l2_offset        = 0;
	pkt_hdr->hw.l3_offset        = ODP_PACKET_OFFSET_INVALID;
	pkt_hdr->hw.l4_offset        = ODP_PACKET_OFFSET_INVALID;
#else
 ...
 * so we need to disable whole generic code.
 */
...
#ifndef HAVE_THUNDERX /* those are inline for thunder */
				 ...
				 const uint8_t **parseptr, uint32_t *offset)
{
...
#ifdef HAVE_THUNDERX
	size_t iplen = odp_be_to_cpu_16(ipv4->tot_len);

	if (odp_unlikely(ihl < ODPH_IPV4HDR_IHL_MIN) ||
	    odp_unlikely(ver != 4) ||
	    (iplen > packet_l3_len(pkt_hdr))) {
		pkt_hdr->hw.error_flags.ip_err = 1;
		return 0;
	}
#else
	...
	*parseptr += ihl * 4;
	...
	if (odp_unlikely(ihl > ODPH_IPV4HDR_IHL_MIN))
#ifdef HAVE_THUNDERX
		pkt_hdr->hw.input_flags.l3_type = NIC_L3TYPE_IPV4_OPTIONS;
#else
...
#ifdef HAVE_THUNDERX
		pkt_hdr->hw.input_flags.l4_type = NIC_L4TYPE_IPFRAG;
#else
...
#ifdef HAVE_THUNDERX
	size_t iplen = odp_be_to_cpu_16(ipv6->payload_len) +
				ODPH_IPV6HDR_LEN;
	...
	if (ipv6->next_hdr == ODPH_IPPROTO_HOPOPTS ||
...
#ifdef HAVE_THUNDERX
		pkt_hdr->hw.input_flags.l3_type = NIC_L3TYPE_IPV6_OPTIONS;
#else
		pkt_hdr->input_flags.ipopt = 1;
#endif
		...
		do  {
			...
			*parseptr += extlen;
			  ...
			  *offset < pkt_hdr->total_len);

		if (*offset >= pkt_hdr->hw.l3_offset +
...
#ifdef HAVE_THUNDERX
		pkt_hdr->hw.error_flags.tcp_err = 1;
#else
			     ...
			     const uint8_t **parseptr, uint32_t *offset)
{
	...
	if (udplen < sizeof(odph_udphdr_t) ||
	    udplen > packet_l4_len(pkt_hdr)) {
		pkt_hdr->hw.error_flags.udp_err = 1;
	}
#else
	if (udplen < sizeof(odph_udphdr_t) ||
...
#ifdef HAVE_THUNDERX
	/* Detect jumbo frames */
	if (pkt_hdr->total_len > ODPH_ETH_LEN_MAX)
...
int _odp_parse_common(odp_packet_hdr_t *pkt_hdr, const uint8_t *ptr)
{
	...
	eth = ptr ? (const odph_ethhdr_t *)ptr :
		(odph_ethhdr_t *)((uint8_t*)(pkt_hdr->buf_hdr.data) + pkt_hdr->segment_offset);
	(void) seglen;
#else
	eth = ptr ? (const odph_ethhdr_t *)ptr :
...
#ifdef HAVE_THUNDERX
		if (ethtype > pkt_hdr->total_len - offset) {
			pkt_hdr->hw.error_flags.snap_len = 1;
			goto parse_exit;
		}
#else
...
#ifdef HAVE_THUNDERX
		pkt_hdr->hw.input_flags.l2_flags.l2_vlan_pressent = 1;
#else
...
#ifdef HAVE_THUNDERX
		pkt_hdr->hw.input_flags.l3_type = NIC_L3TYPE_IPV4;
#else
...
#ifdef HAVE_THUNDERX
		pkt_hdr->hw.input_flags.l3_type = NIC_L3TYPE_IPV6;
#else
...
#ifdef HAVE_THUNDERX
		if (NIC_L4TYPE_IPFRAG != pkt_hdr->hw.input_flags.l4_type) {
			pkt_hdr->hw.input_flags.l4_type = NIC_L4TYPE_TCP;
...
#ifdef HAVE_THUNDERX
		if (NIC_L4TYPE_IPFRAG != pkt_hdr->hw.input_flags.l4_type) {
			pkt_hdr->hw.input_flags.l4_type = NIC_L4TYPE_UDP;
...
#ifdef HAVE_THUNDERX
		if (NIC_L4TYPE_IPFRAG != pkt_hdr->hw.input_flags.l4_type) {
			pkt_hdr->hw.input_flags.l4_type = NIC_L4TYPE_IPSEC_ESP;
...
parse_exit:
	pkt_hdr->input_flags.parsed_all = 1;
#ifdef HAVE_THUNDERX
	return pkt_hdr->hw.error_flags.all != 0;
#else
